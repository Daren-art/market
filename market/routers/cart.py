...
def add_to_cart(product_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    # ИСПРАВЛЕНИЕ 3: проверяем, что товар существует
...
def cart_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        ...
        context={"request": request, "products": products, "total": total, "csrf_token": get_csrf_token(request)}
    )
...
def checkout(request: Request, db: Session = Depends(get_db)):
    ...
    # Фильтруем только корзинные позиции с реально существующими товарами
    valid_items = [
        item for item in cart_items
        if db.query(Product).filter(Product.id == item.product_id).first()
    ]

    # Удаляем "мусорные" записи с несуществующими товарами
    invalid_items = [item for item in cart_items if item not in valid_items]
